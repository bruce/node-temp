...
const mkdir = (affixes, callback) => {
  ...
  let dirMode = getMode(affixes, '0700');
  let dirPath = generateName(affixes, 'd-');
  mkdirp(dirPath, dirMode, (err) => {
    if (!err) {
...
const open = (affixes, callback) => {
  ...
  const fileMode = getMode(affixes, '0600');
  fs.open(path, RDWR_EXCL, fileMode, (err, fd) => {
    if (!err) {
...
const openSync = (affixes) => {
  const path = generateName(affixes, 'f-');
  const fileMode = getMode(affixes, '0600');
  let fd = fs.openSync(path, RDWR_EXCL, fileMode);
  deleteFileOnExit(path);
  return { path, fd };
}
...
const createWriteStream =  (affixes) => {
  ...
  const fileMode = getMode(affixes, '0600');
  let stream = fs.createWriteStream(path, { flags: RDWR_EXCL, mode: fileMode });
  deleteFileOnExit(path);
  return stream;
}
